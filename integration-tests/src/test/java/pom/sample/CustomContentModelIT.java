...
public class CustomContentModelIT extends AbstractAlfrescoIT {
    ...
    private NodeRef createNode(String name, QName type, Map<QName, Serializable> properties) {
        NodeRef projectFolderNodeRef = getCompanyHomeNodeRef();
        QName associationType = ContentModel.ASSOC_CONTAINS;
        QName associationQName = QName.createQName(NamespaceService.CONTENT_MODEL_1_0_URI,
                QName.createValidLocalName(name));
        properties.put(ContentModel.PROP_NAME, name);
        ChildAssociationRef projectChildAssocRef = getServiceRegistry().getNodeService().createNode(
                projectFolderNodeRef, associationType, associationQName, type, properties);

        return projectChildAssocRef.getChildRef();
    }
